...
总体来说，我认为这个课的课程安排是非常糟糕的。数据库是一个非常有价值的方向，涵盖了计算机科学的几乎各个领域，本来应该是一门提高学生在系统方向各方面能力的课程，但是课程的 PPT 设计非常的简陋粗糙，没法给学生任何有趣的感觉，而且课堂讲授的东西与最后的课程评价几乎没有任何关系，这样的结果必然是大部分学生不会去课堂上听课。在我看来，不去提升课堂质量，而是在微信群里表扬去上课的同学，发合影，是非常幼稚的行为。

另一方面，课程的作业设计也非常的离谱。前两个小作业描述模糊，与课堂讲述的硬核内容几乎没有任何关系，同时两个小作业与最后的大作业也几乎没有任何关系，给人的感觉就是完全为了凑一个部分分而给出的作业。在大作业方面，让学生从零实现一个数据库固然是对工程实践能力非常有挑战性，但是也应该考虑这样的工作量对于大部分学生来说是不是过于高了。事实上据我所知，大部分学生也是「各凭本领」才完成了最后的实现，能否真的达到对工程能力的锻炼也是个难以确定的问题。现在计算机系的大部分课程都是采用代码填空的形式，这样既可以在一定程度上考察实现，又可以很方便的设计单元测试，是否也应该考虑将大作业替换成这样的形式呢。或者只是设计一个测试框架，也可以免除掉最后繁琐的逐组检查的过程，毫无疑问对于学生和助教来说都是更好的体验。

在整个大作业的布置过程中，必做功能与选做功能描述不清反复变化，最后的验收标准迟迟无法确定，验收过程也异常混乱，给出的验收规则让人摸不到头脑，就更不用提后面测试数据泄露的问题了。我觉得这不应该是中国最好的大学数据库课程的现状，如果冒犯到了助教和老师也望理解。
